...
class ApiTasks(TaskSet):
    def on_start(self):
        ...
        self.user_data_list_dict=CSVDictFeeder(self.test_user_data_csv).read_csv_dataFile()
        self.lst_user_id=sorted([ k['id'] for k in self.user_data_list_dict])
    ...
    def update_api_user(self):
        if len(self.lst_user_id) > 0:
            ...
            data_d={'status': 'Active'}   # need to updated user info dictionary
            self.client.put(url, headers=headers,data=json.dumps(data_d))


        else:
            print("All of user has been updated ! ")
            logging.error("All of user  has been updated ! Locust will stop it !")
...
#
#     print(user_data_list_dict[0])
#
#     lst_user_id = sorted([k['id'] for k in user_data_list_dict])
#     print(lst_user_id)
#     print("Total user updated : {}".format(len(lst_user_id)))
#
